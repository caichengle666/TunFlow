...
func (a *App) startConfigWatcherLocked() {
	if a.configWatchStop != nil { return }
...
func (a *App) configWatchLoop(stop <-chan struct{}) {
	...
	for {
		select {
		case <-stop: return
		case <-ticker.C: a.reloadConfigFromDisk()
		}
	}
}

func readConfig(path string) (Config, []byte, error) {
	data, err := os.ReadFile(path); if err != nil { return Config{}, nil, err }
	clean := trimBOMSpace(data)
	if len(clean) == 0 { return Config{}, nil, errors.New("配置文件为空") }
	var cfg Config
	if err := json.Unmarshal(clean, &cfg); err != nil { return Config{}, nil, fmt.Errorf("JSON 配置格式无效: %w", err) }
	cfg.Proxy = normalizeProxy(cfg.Proxy)
	if err := normalizeConfig(&cfg); err != nil { return Config{}, nil, err }
	return cfg, clean, nil
}

func trimBOMSpace(data []byte) []byte {
	for len(data) > 0 && (data[0] == ' ' || data[0] == '\t' || data[0] == '\r' || data[0] == '\n') { data = data[1:] }
	if len(data) >= 3 && data[0] == 0xEF && data[1] == 0xBB && data[2] == 0xBF { data = data[3:] }
...
func (a *App) reloadConfigFromDisk() {
	cfg, data, err := readConfig(a.configPath()); if err != nil { return }
	sum := checksum(data)
	a.mu.Lock()
	if sum == a.configDiskChecksum { a.mu.Unlock(); return }
	cfg2, data2, err2 := readConfig(a.configPath())
	if err2 != nil || checksum(data2) != sum || !reflect.DeepEqual(cfg, cfg2) { a.mu.Unlock(); return }
