...
func (a *App) startup(ctx context.Context) {
	...
	if err := a.loadConfigLocked(); err != nil {
		if os.IsNotExist(err) {
			if err := normalizeConfig(&a.cfg); err != nil {
				a.err = err
				a.configLoaded = false
			} else if err := a.writeConfigLocked(); err != nil {
				a.err = err
				a.configLoaded = false
			} else {
				a.err = nil
...
func (a *App) GetStatus() Status {
	a.mu.Lock()
	defer a.mu.Unlock()
	return Status{Running: engine.Running(), Proxy: a.cfg.Proxy, Device: a.cfg.Device, Mode: a.cfg.Mode, AutoRoute: a.cfg.AutoRoute, RouteReady: a.up.active, LastError: errString(a.err), StartWithWindows: a.cfg.StartWithWindows}
}
...
func (a *App) SaveConfig(cfg Config) error {
	...
	if engine.Running() {
		a.mu.Lock()
		err := a.applyRunningConfigLocked(old)
		a.err = err
		a.mu.Unlock()
		if err != nil {
			return fmt.Errorf("配置已保存，但运行中的核心未能完全应用: %w", err)
		}
	}
	return nil
}
...
func (a *App) stopLocked() error {
	if a.up.active {
		if err := a.teardownRoutesLocked(); err != nil {
			return fmt.Errorf("清理 Windows 路由失败: %w", err)
		}
	}
...
func (a *App) applyRunningConfigLocked(old Config) error {
	...
	newCfg := a.cfg
	if strings.TrimSpace(newCfg.Proxy) == "" {
		return errors.New("运行中的 TunFlow 不能清空 SOCKS5 地址")
	}
	if old.Device != newCfg.Device || old.Interface != newCfg.Interface || old.AutoRoute != newCfg.AutoRoute || old.Proxy != newCfg.Proxy {
		...
		if err := a.startLocked(); err != nil {
			a.cfg = old
			_ = a.startLocked()
			return fmt.Errorf("重启核心应用新配置失败: %w", err)
		}
		return nil
	}
	if err := engine.Reload(a.buildEngineKeyLocked(newCfg)); err != nil {
		a.cfg = old
		return fmt.Errorf("热更新核心失败: %w", err)
	}
	return nil
}
...
func (a *App) buildEngineKeyLocked(cfg Config) *engine.Key {
	...
	if iface == "" {
		iface = a.runtimeInterface
	}
	geo := cfg.GeoIPFile
	if !filepath.IsAbs(geo) {
		if p, ok := bundledDataPath(geo); ok {
			geo = p
		}
	}
...
func cloneConfig(cfg Config) Config {
	out := cfg
	out.DirectCIDRs = append([]string(nil), cfg.DirectCIDRs...)
	out.DirectRules = append([]string(nil), cfg.DirectRules...)
	out.ProxyRules = append([]string(nil), cfg.ProxyRules...)
	return out
}
...
func (a *App) loadConfigLocked() error {
	...
	data = bytesTrimBOMSpace(data)
	if len(data) == 0 {
		return errors.New("配置文件为空")
	}
	var cfg Config
	if err := json.Unmarshal(data, &cfg); err != nil {
		return fmt.Errorf("JSON 配置格式无效: %w", err)
	}
	cfg.Proxy = normalizeProxy(cfg.Proxy)
	if err := normalizeConfig(&cfg); err != nil {
		return err
...
func bytesTrimBOMSpace(data []byte) []byte {
...
func (a *App) writeConfigLocked() error {
	...
	if err != nil {
		return fmt.Errorf("保存后读取配置失败: %w", err)
	}
	if string(stored) != string(data) {
		return errors.New("配置保存校验失败")
	}
	a.configDiskChecksum = checksumBytes(data)
	return nil
}
...
func (a *App) refreshConfigChecksumLocked() {
...
func (a *App) updateRuleFile(source, name, label string) error {
	...
	tmp, err := downloadRuleFile(client, source, dir, name)
	if err != nil {
		return fmt.Errorf("更新 %s 失败: %w", label, err)
	}
	defer os.Remove(tmp)
	if err := replaceRuleFile(tmp, filepath.Join(dir, name)); err != nil {
...
func checkProxyEndpoint(raw string) error {
	raw = normalizeProxy(raw)
	u, err := url.Parse(raw)
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("SOCKS5 入口不可达: %w", err)
	}
	_ = conn.Close()
	return nil
}
