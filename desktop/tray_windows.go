...
func (a *App) startSystemTray() {
	trayOnce.Do(func() {
		startLoop, endLoop := systray.RunWithExternalLoop(func() {
			...
			// Keep tray UI calls on one goroutine. In particular, do not mutate
			// menu state every second while Windows is opening the context menu.
			go func() {
