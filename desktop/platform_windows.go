...
func runPowerShellText(script string) (string, error) {
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			if msg == "" {
				msg = strings.TrimSpace(string(exitErr.ProcessState.String()))
			}
...
func defaultGateway(interfaceName string) (string, error) {
	...
	if strings.TrimSpace(interfaceName) != "" {
		filter = fmt.Sprintf(" | Where-Object { $_.InterfaceAlias -eq %s }", psQuote(interfaceName))
	}
	...
	value, err := runPowerShellText(script)
	if err != nil {
		return "", err
	}
	if net.ParseIP(value) == nil {
		return "", fmt.Errorf("PowerShell 返回的默认网关无效: %q", value)
	}
	return value, nil
}
...
func physicalInterfaceIndex(gateway string) (string, error) {
	...
	if err != nil || idx <= 0 {
		return "", fmt.Errorf("物理网卡索引无效: %q", value)
	}
...
func (a *App) selectRuntimeInterfaceLocked() error {
	if name := strings.TrimSpace(a.cfg.Interface); name != "" {
...
func readTunConfig(name string) (tunConfig, error) {
	...
	if len(parts) < 4 {
		return tunConfig{}, fmt.Errorf("读取 TUN 配置返回值无效: %q", value)
	}
	prefix, err := strconv.Atoi(parts[2])
	if err != nil {
		return tunConfig{}, fmt.Errorf("TUN 前缀长度无效: %q", parts[2])
	}
...
func windowsTunDefaultRouteReady() (bool, error) {
	...
	for _, line := range strings.Split(string(out), "\n") {
		fields := strings.Fields(line)
		if len(fields) >= 4 && fields[0] == "0.0.0.0" && fields[1] == "0.0.0.0" && (fields[2] == "198.18.0.1" || fields[3] == "198.18.0.1") {
			return true, nil
		}
	}
	return false, nil
...
func (a *App) setupRoutesLocked() error {
	...
	cleanup := func() {
		for _, ip := range added {
			_ = runWindows("route", "DELETE", ip, "MASK", "255.255.255.255", "IF", physicalIndex)
		}
