...
func (r *Proxy) DialContext(ctx context.Context, metadata *M.Metadata) (net.Conn, error) {
	if r.useDirect(metadata) { return r.direct.DialContext(ctx, metadata) }
...
func (r *Proxy) DialUDP(metadata *M.Metadata) (net.PacketConn, error) {
	if r.useDirect(metadata) { return r.direct.DialUDP(metadata) }
...
func (c *statsConn) Read(b []byte) (int, error) { n, err := c.Conn.Read(b); c.c.downloaded(n); return n, err }
func (c *statsConn) Write(b []byte) (int, error) { n, err := c.Conn.Write(b); c.c.uploaded(n); return n, err }

func (c *statsConn) CloseRead() error {
	if v, ok := c.Conn.(interface{ CloseRead() error }); ok { return v.CloseRead() }
	return nil
}
func (c *statsConn) CloseWrite() error {
	if v, ok := c.Conn.(interface{ CloseWrite() error }); ok { return v.CloseWrite() }
	return nil
}
...
func (c *statsPacketConn) ReadFrom(b []byte) (int, net.Addr, error) { n, addr, err := c.PacketConn.ReadFrom(b); c.c.downloaded(n); return n, addr, err }
func (c *statsPacketConn) WriteTo(b []byte, addr net.Addr) (int, error) { n, err := c.PacketConn.WriteTo(b, addr); c.c.uploaded(n); return n, err }
